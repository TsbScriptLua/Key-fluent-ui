...
-- //WARNING IF YOU WANT TO SKIDS MY SCRIPT PLS CREDITS A ELEMENT//--
...
local Window = Rayfield:CreateWindow({
    Name = "▶ Aimbot Universal | Faheem ◀",
    LoadingTitle = "Load Hub....",
    LoadingSubtitle = "Please Be patient may take a while..",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = "Faheem",
        FileName = "byFaheem"
...
local function checkTeam(player)
    ...
    return false
end --This its where the universal location workspace ("check:Gameservice")
...
local smoothingslider = Aimbot:CreateSlider({
    Name = "Smoothing ("Head:Pose") when Aiming",
    Range = {0, 100},
    Increment = 1,
    CurrentValue = 5,
    Flag = "Smoothing",
...
local teamchecktoggle = Aimbot:CreateToggle({
    ...
    Flag = "TeamCheck",
    Callback = function(Value)
        teamCheck = Value --//Checking team its if okay//--
    end
})

local healthchecktoggle = Aimbot:CreateToggle({
    Name = "Health Check (Visible Health)",
    CurrentValue = false,
    Flag = "HealthCheck",
    Callback = function(Value)
        healthCheck = Value
