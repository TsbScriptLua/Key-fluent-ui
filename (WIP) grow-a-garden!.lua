...
-- Please give credits if you gonna skid this (Faheem)
-- //(If you not the owner i want you to make credits down there..
...
local autoSection = mainTab:Section("Features")
...
-- Auto Sell based on different conditions of the task..
task.spawn(function()
	while task.wait() do
		if autoSell then
			...
			if sellBasedOnSlider and holdableCount >= sellAtCount then
				teleportAndSell()  -- Trigger teleportation and selling the ()selection
			end
		end
	end
end)
...
local function autoBuySeedsFunction()
	local shopUI = player:FindFirstChild("PlayerGui"):FindFirstChild("Seed_Shop")
	if not shopUI then return end -- if there bug pls rejoin...
